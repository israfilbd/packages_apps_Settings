...
public class SettingsHomepageActivity extends FragmentActivity implements
    ...
    private static final int DELAY_BETWEEN_CHARACTERS = 200;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        int currentTextColor = sharedPreferences.getInt(TEXT_COLOR_KEY, Color.WHITE);
        userTitleTextView.setTextColor(currentTextColor);

        String ownerName = getOwnerName();
        userTitleTextView.setText("");
        startTypingAnimation(ownerName, userTitleTextView);

        userTitleTextView.setOnLongClickListener(new View.OnLongClickListener() {
            @Override
            public boolean onLongClick(View v) {
                int newTextColor = (userTitleTextView.getCurrentTextColor() == Color.WHITE) ?
                        Color.BLACK : Color.WHITE;
                userTitleTextView.setTextColor(newTextColor);

                SharedPreferences.Editor editor = sharedPreferences.edit();
                editor.putInt(TEXT_COLOR_KEY, newTextColor);
                editor.apply();
        ...
        LinearLayout clickableLayout = findViewById(R.id.infinity_x);
        clickableLayout.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent();
                intent.setClassName("com.android.settings", "com.android.settings.Settings$InfinitySuiteActivity");
                startActivity(intent);
            }
        });
    ...
    private void startTypingAnimation(final String text, final TextView textView) {
        final Handler handler = new Handler();
        final boolean initialCursorVisibility = false;
        
        textView.setGravity(Gravity.BOTTOM | Gravity.START);

        handler.postDelayed(new Runnable() {
            private int index = 0;
            private boolean cursorVisible = initialCursorVisibility;

            @Override
            public void run() {
                StringBuilder currentText = new StringBuilder(text.substring(0, index));
                if (cursorVisible) {
		    currentText.append("|");
		} else {
		    currentText.append(" ");
           	...
           	textView.setText(currentText.toString());           	
                ...
                if (index <= text.length()) {
                    handler.postDelayed(this, DELAY_BETWEEN_CHARACTERS);
                } else {
    ...
    private void showSuggestionFragment(boolean scrollNeeded) {
        ...
        if (fragmentClass == null) {
            Log.e("showSuggestionFragment", "Fragment class not found");
            return;
        }
        ...
        if (mHomepageView == null) {
            Log.e("showSuggestionFragment", "Homepage view (settings_homepage_container) not found");
            return;
        }
        ...
        // Check if mIsEmbeddingActivityEnabled and if mTwoPaneSuggestionView is found
        if (mIsEmbeddingActivityEnabled) {
            if (mTwoPaneSuggestionView == null) {
                Log.e("showSuggestionFragment", "Two-pane suggestion view (two_pane_suggestion_content) not found");
                return;
            }
    ...
    private String getOwnerName(){
        ...
        String ownerName = userInfo.name != null ? userInfo.name : getString(R.string.default_user);
        return "Hi " + ownerName;
    }
